...
export default async function InstructorDashboardPage() {
  ...
  // ---------------------------------
  // MVP placeholders (შემდეგ ეტაპზე DB)
  // ---------------------------------
  const stats = {
    myCourses: "—",
    published: "—",
    ...
    <main className="container-page section-pad">
      ...
      <section className="mt-6">
        ...
        <div className="mt-3 divide-y divide-white/10 rounded-xl border border-white/10 bg-white/5">
          {drafts.map((d) => (
            <Link
              key={d.id}
              href={`/dashboard/courses?status=draft&q=${encodeURIComponent(
                d.title
              )}`}
      ...
      <section className="mt-6">
        ...
        <ol className="mt-3 space-y-2 text-sm text-white/70">
          <li>1) შექმენი ან დაასრულე კურსის აღწერა და cover</li>
          <li>2) დაამატე sections / lessons სტრუქტურა</li>
          <li>3) ატვირთე ვიდეო ან დაამატე YouTube / Vimeo ლინკი</li>
          <li>4) მიამაგრე PDF ან სხვა ფაილები lesson-ზე</li>
